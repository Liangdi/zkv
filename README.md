...
- 🔄 **口令缓存 agent** — 无感缓存已派生密钥:连续命令只输一次口令、跳过 Argon2id;闲置自动清零退出(ssh-agent/sudo 风格,密钥**只在内存、绝不落盘**)。
...
# 口令缓存 agent(默认开启;详见下方「口令缓存 agent」):
zkv agent status · zkv agent stop · zkv lock     # 查看状态 / 停止 / 清空缓存密钥
```
...
```bash
zkv ls ~/my.zkv        # 首次:正常输口令 → agent 自动在后台拉起、缓存密钥
zkv otp 1 ~/my.zkv     # 之后(默认 5 分钟内):不再问口令、近乎瞬时
zkv cp 2 ~/my.zkv
```

- **自动**:首次需要口令时自我 spawn,闲置超过 `ZKV_LOCK_SECS`(默认 300 秒,与 TUI 自动锁同一个变量)自动清密钥退出。`passwd` 改口令后缓存自动失效。
- **安全**:派生密钥**只在 agent 进程内存**(`Zeroizing`,drop 清零),**绝不落盘**;经本地 0600 Unix socket 传给同 uid 的客户端(同 ssh-agent);访问控制 = 0700 私有目录(`$XDG_RUNTIME_DIR` 优先)。任何异常(连不上 / 版本不符 / 库被外部改了口令)都**静默回退**到正常输口令,绝不卡死或损坏。
- **控制 / 关闭**:

  ```bash
  zkv agent status          # 进程 pid / socket / 已缓存库 / 剩余空闲
  zkv agent stop            # 停止 agent(清密钥退出)
  zkv lock                  # 清空所有缓存密钥(立即需重新输口令)
  zkv ls --no-agent ...     # 或 ZKV_NO_AGENT=1:本次禁用缓存
  ZKV_LOCK_SECS=0           # 永久禁用(TTL=0,等价全局 opt-out)
  ```

> agent 仅在 **Unix**(Linux/macOS/WSL)生效;其他平台为 no-op,退回每命令输口令。
...
> **自动锁定**:TUI 无操作超过 `ZKV_LOCK_SECS`(默认 300 秒,`0` 禁用)自动上锁,可直接原地重输口令恢复。CLI 的口令缓存 agent 也复用此变量作为闲置 TTL(见上方「口令缓存 agent」)。
...
**威胁模型**
- ✅ 防御:`.zkv` 文件被离线窃取后只能暴力破解口令(Argon2id 拉高成本);明文不落盘;临时文件 0600 且名取自 CSPRNG;元数据(条目数、标签名等)整体加密不可见;复制密码 20s 自动清空剪贴板;闲置自动锁定;口令缓存 agent 的密钥只在内存、绝不落盘,经 0600 本地 socket 传同 uid 客户端。
...
error(L0) → crypto/model/totp(L1) → db/vault(L2) → store/search/clipboard(L3) → app(L4) → ui(L5) → main(L6)
                                                                   ↘ cli(无头前端,与 ui 平行) · agent(Unix 守护进程,缓存派生密钥)
```
...
```bash
cargo test             # 单元 / 集成测试(215 passed)
cargo clippy --all-targets  # 0 warning
just e2e               # PTY 端到端(驱动真实二进制,7 用例)
...
- [x] 口令缓存 agent(无感缓存派生密钥,跳过 Argon2id;ssh-agent/sudo 风格)
- [ ] KeePass 导入 / 导出
